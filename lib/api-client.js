...
module.exports = {
    ...
    '2fa': {
        ...
        checkTotp(user, token, totpNonce, sess, ip, callback) {
            _exec('post', '/users/{user}/2fa/totp/check', { user: user.id }, { token, totpNonce, sess, ip }, user.token, callback);
        },
        ...
        setupWebAuthn(user, data, callback) {
            _exec('post', '/users/{user}/2fa/webauthn/registration-challenge', { user: user.id }, data, user.token, callback);
        },

        enableWebAuthn(user, requestData, callback) {
            _exec('post', '/users/{user}/2fa/webauthn/registration-attestation', { user: user.id }, requestData, user.token, callback);
        },

        listWebAuthn(user, sess, ip, callback) {
            _exec('get', '/users/{user}/2fa/webauthn/credentials?sess={sess}&ip={ip}', { user: user.id, sess, ip }, false, user.token, callback);
        },

        deleteWebAuthnCredential(user, credential, sess, ip, callback) {
            _exec(
                'del',
                '/users/{user}/2fa/webauthn/credentials/{credential}?sess={sess}&ip={ip}',
                { user: user.id, credential, sess, ip },
                false,
                user.token,
                callback
            );
        ...
        disableWebAuthn(user, sess, ip, callback) {
            this.listWebAuthn(user, sess, ip, (err, response) => {
                if (err) {
                    return callback(err);
                }

                let credentials = (response && response.credentials) || [];
                if (!credentials.length) {
                    return callback(null, { success: true, deleted: false });
                }

                let pending = credentials.length;
                let deleted = false;
                let done = false;

                credentials.forEach(credential => {
                    this.deleteWebAuthnCredential(user, credential.id, sess, ip, (err, response) => {
                        if (done) {
                            return;
                        }
                        ...
                        deleted = deleted || !!(response && response.deleted);

                        if (!--pending) {
                            callback(null, { success: true, deleted });
                        }
                    });
                });
            });
        ...
        startWebAuthn(user, requestData, callback) {
            _exec('post', '/users/{user}/2fa/webauthn/authentication-challenge', { user: user.id }, requestData, user.token, callback);
        },

        checkWebAuthn(user, requestData, callback) {
            _exec('post', '/users/{user}/2fa/webauthn/authentication-assertion', { user: user.id }, requestData, user.token, callback);
        }
    },
