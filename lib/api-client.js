    ...
    '2fa': {
        ...
        checkTotp(user, token, totpNonce, sess, ip, callback) {
            _exec(
                ...
                { user: user.id },
                { token, totpNonce, sess, ip },
                _getTwoFactorAuthToken(user),
                callback
            );
        },
            ...
            this.listWebAuthn(user, sess, ip, (err, response) => {
                ...
                credentials.forEach(credential => {
                    this.deleteWebAuthnCredential(user, credential.id, sess, ip, (err, response) => {
                        ...
                        if (!--pending) {
                            return callback(null, { success: true, deleted });
                        }
                    });
                });
            });
...
function _getTwoFactorAuthToken(user) {
    return (user && user.token) || config.api.accessToken;
}
